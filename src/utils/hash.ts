...
function ToShortId(addrBytes: number[]): number[] {
  if (addrBytes.length !== ripemd160Size)
    throw new Error(`expected ${ripemd160Size} bytes but got ${addrBytes.length}`);
  return addrBytes;
}
...
export function ToValidatorConfigHash(
  ...
  const validatorConfig = new Uint8Array(
    pChainPublicKeyHash.length + nodeIDHash.length + nodeWeightHash.length + nodeDurationHash.length
  );
