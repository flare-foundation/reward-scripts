...
export class EventProcessorService {
  ...
  public async processEvents(
    firstBlockToProcess: number,
    rps: number,
    batchSize: number,
    votingLength: number,
    votingStartTs: number,
    epochId: number
  ) {
    ...
    const votingEndTs = votingStartTs + votingLength;
    const lastBlock = (await this.contractService.web3.eth.getBlockNumber()) - 1;
    const lastBlockTs = (await this.contractService.web3.eth.getBlock(lastBlock)).timestamp as number;
    ...
    while (nextBlockToProcessTs <= votingEndTs) {
      try {
        ...
        if (rps != Infinity) {
          ...
          const contractEventBatches = await this.contractService.getEventsFromBlockForContract(
