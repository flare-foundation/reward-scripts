...
- **Indexer API key** — `API_PATH_{NETWORK}` env var (e.g. `API_PATH_FLARE`) overrides the config `API_PATH` and carries the key in the query string, e.g. `https://flare-p-chain-indexer.flare.network?x-apikey=...`. The key lifts the request limit (measured: 107 requests at 239 req/min, no 429), so a keyed path also sets `indexerRequestsPerSecond` to `Infinity`, same shape as `RPC_URL_{NETWORK}`. An explicit `INDEXER_REQUESTS_PER_SECOND` env var overrides both — use it to dial the rate back from CI without a code change. The key must go in the **query string**; `x-apikey` and `X-API-Key` headers are ignored
