...
2. **`distribute-testnet-rewards`** (`src/distributeTestnetRewards.ts`) — distributes the current window's *already-committed* payout file on-chain, guarded for idempotency by the committed distributions file `generated-files/{network}/validator-rewards/distributions.json` (the public RPC caps `getPastEvents` at 30 blocks, so event-scan idempotency is infeasible). Key-gated by `DISTRIBUTOR_PRIVATE_KEY`. **First run self-initializes**: if the distributions file does not exist it records every window from `REWARD_EPOCH` through the current distribution epoch as already-paid (so windows already paid by the old single-stage code or manually are never re-paid) and distributes nothing — distribution then proceeds forward-only. CI commits + pushes the distributions file after. Manual catch-up for one past window: `--lastEpoch <D>` distributes the window ending at `D` (even if self-init wrote it as an unpaid placeholder), and `--dry-run` previews addresses/amounts without sending.
...
- **Indexer throttling** — unauthenticated, the p-chain indexer (`API_PATH`) allows **60 requests/min** (measured: request 61 refused with `Retry-After: 60`) and caps `limit` at 100, so a full delegator sweep (100+ pages) cannot fit in one minute. `getActiveStakes` paces at `INDEXER_REQUESTS_PER_SECOND` (env var, else config file, else 1 — request latency makes the effective rate ~0.85/s) and retries 429/5xx through `httpWithRetry`, honouring `Retry-After`. Separate from `MAX_REQUESTS_PER_SECOND`/`RPC_URL_{NETWORK}`, which pace the RPC only — never reuse the RPC rate here
- **Indexer API key** — `INDEXER_API_KEY_{NETWORK}` env var (e.g. `INDEXER_API_KEY_FLARE`), appended as the `x-apikey` **query param** at request time (the indexer ignores `x-apikey`/`X-API-Key` headers). Keep it out of URLs and config files: GitLab refuses to mask a value containing `?`, so a key embedded in a URL cannot be a masked CI variable. Setting it lifts the request limit (measured: 107 requests at 239 req/min, no 429), so it also sets `indexerRequestsPerSecond` to `Infinity`; an explicit `INDEXER_REQUESTS_PER_SECOND` overrides that, to dial the rate back from CI without a code change
- **Indexer host override** — `API_PATH_{NETWORK}` env var (e.g. `API_PATH_FLARE`) overrides the config `API_PATH`, for failing over to a mirror without a merge. Not a secret and, unlike `RPC_URL_{NETWORK}`, it does **not** change the request rate — only the key does
